...
export async function generateMetadata({ params }) {
  ...
  try {
    const res1 = await fetch(`${server}/shohou/${eid}_ind.json`, { next: { revalidate: false } });
...
export default async function NdbEidPage({ params }) {
  ...
  const graphList = [
    ...
    { value: 'sum', label: '処方数', unit: '' },
    { value: 'slf', label: '売上', unit: '円' },
    { value: 'dff', label: '前年比', unit: '%' },
    { value: 'ykk', label: '薬価', unit: '円' },
  ];

  let ssg1, ssg2;
  try {
    const [res1, res2] = await Promise.all([
      fetch(`${server}/shohou/${eid}_ind.json`, { next: { revalidate: false } }),
      fetch(`${server}/shohou/${eid}_gen.json`, { next: { revalidate: false } }),
    ]);
