...
export async function generateMetadata({ params }) {
  ...
  try {
    const res2 = await fetch(`${server}/shohou/${id2}.json`, { next: { revalidate: false } });
    if (!res2.ok) return { title: `処方薬データ ${id2}` };
    const ssg2 = await res2.json();
    const con_name = ssg2.def.dcn;
    return {
...
export default async function PrescriptionPage({ params }) {
  ...
  let res2;
  try {
    const response = await fetch(`${server}/shohou/${id2}.json`, { next: { revalidate: false } });
    res2 = await response.json();
  } catch (e) {
    return <div>データが見つかりません。</div>;
  }

  const con_name = res2.def.dcn;
